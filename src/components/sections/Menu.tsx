...
const categories: { key: MenuCategory; label: string }[] = [
  { key: 'appetizers', label: 'Entrées' },
  { key: 'mains', label: 'Plats Principaux' },
  { key: 'desserts', label: 'Desserts' },
  { key: 'wine', label: 'Sélection de Vins' },
];
...
const Menu = () => {
  ...
  return (
    <section
      id="menu"
      ref={sectionRef}
      className="section-padding relative overflow-hidden bg-offwhite"
    >
      ...
      <div className="container mx-auto px-6 lg:px-12 relative z-10">
          ...
          <div className="relative flex flex-wrap justify-center items-center gap-1 md:gap-0">
            ...
            {categories.map((category) => (
              <button
                ...
                className="group relative px-6 md:px-8 py-4"
              >
                <span className={`relative z-10 font-sans text-sm md:text-base tracking-[0.08em] uppercase transition-all duration-500 ${
                ...
                {/* Active underline */}
                <span className={`absolute bottom-2 left-1/2 -translate-x-1/2 h-0.5 bg-gold rounded-full transition-all duration-500 ${
                  activeCategory === category.key ? 'w-8' : 'w-0 group-hover:w-4 group-hover:bg-charcoal/20'
                }`} />
              </button>
            ))}
          </div>
        ...
        >
          {isWineCategory ? (
            /* Wine List - Modern glass morphism */
            <div className="max-w-4xl mx-auto">
              <div className="bg-charcoal/5 backdrop-blur-md border border-charcoal/10 rounded-2xl overflow-hidden">
                {menuData.wine.map((item, index) => (
                  <div
                    key={item.name}
                    className={`group relative flex items-center justify-between p-6 md:p-8 transition-all duration-500 hover:bg-charcoal/10 ${
                      index !== menuData.wine.length - 1 ? 'border-b border-charcoal/10' : ''
                    }`}
                  >
                    {/* Wine Icon with glow */}
            ...
            /* Food Items - Modern card design */
            <div className="grid md:grid-cols-2 gap-6 lg:gap-8">
              {menuData[activeCategory].map((item, index) => (
                <div
                  key={item.name}
                  className="group relative rounded-2xl overflow-hidden transition-all duration-700 bg-white shadow-lg"
                ...
                >
                  ...
                  <div className="relative">
                    {/* Image Section */}
                    {item.image && (
                      <div className="relative h-52 lg:h-64 overflow-hidden rounded-t-2xl">
                        <img
                          src={item.image}
                          alt={item.name}
                    ...
                    <div className="relative p-6 lg:p-8 bg-charcoal">
                      ...
                      <a 
                        href={`/menu/${activeCategory === 'appetizers' ? 'entrees' : activeCategory === 'mains' ? 'plats' : 'desserts'}`}
                        className="mt-6 flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-2 group-hover:translate-y-0"
                      >
        ...
        >
          ...
          <p className="font-sans text-xs text-charcoal/50 tracking-wider italic">
            Inspiré par les saisons. Prix susceptibles de changer. Veuillez nous informer de tout régime alimentaire particulier.
          </p>
        </div>
      </div>
    </section>
